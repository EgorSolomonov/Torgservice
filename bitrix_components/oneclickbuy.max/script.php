...
if (isset($query['uploadfiles']) && isset($query['orderID'])) {
	...
	if ($orderID) {
		...
		// Кастом для получения Артикула товара 
		...
		while ($arProps = $dbBasketItems->Fetch()) {
			...
			$productIdForArtictle = $arProps['PRODUCT_ID'];

			$arProduct = CIBlockElement::GetByID($productIdForArtictle)->GetNext();
		...
		// Кастом для получения Артикула товара end
		...
		while ($arBasketItem = $resBasketItems->Fetch()) {
			...
			if ($arBasketItem['CAN_BUY'] === 'Y') {
				...
				$orderList .= GetMessage('ITEM_NAME') . $arBasketItem['NAME']
					...
					. GetMessage('ITEM_QTY') . intval($arBasketItem['QUANTITY'])
					. GetMessage('ITEM_TOTAL') . CCurrencyLang::CurrencyFormat($curPrice, $newOrder['CURRENCY'], true) . " Артикул: " .  $article . "\n";
			}
		}
